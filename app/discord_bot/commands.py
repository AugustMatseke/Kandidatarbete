...
def modifyevent(user_id: str, name: str, time: str, location: str):
    ...
    for participant in event[4]:
        gcal.modify_event(user_id, events[name][participant], name, time, location=location)
...
def leaveevent(user_id: str, name: str):
    ...
    if db.leaveevent(name, str(user_id)) and gcal.remove_event(user_id, events[name][user_id]):
